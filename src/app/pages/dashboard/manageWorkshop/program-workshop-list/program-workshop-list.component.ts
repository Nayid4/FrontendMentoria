...
@Component({
  selector: 'app-program-workshop-list',
  imports: [
    TableModule,
    CommonModule,
    ButtonModule,
  ...
  templateUrl: './program-workshop-list.component.html',
  styleUrl: './program-workshop-list.component.css'
})
export class ProgramWorkshopListComponent {
  search: string = '';
  first = 0;
  rows = 10;
  ...
  get volunteeringPrograms(): ProgramResponse[] {
    const filtrados = this.pageList.elements.filter(
      p => p.type && p.type.toLowerCase() === 'taller'
    );
